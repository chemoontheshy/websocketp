...
vsnc::vnet::Pubisher::Pubisher(const uint16_t port)
{
	try {
		...
		m_iServer.set_error_channels(websocketpp::log::alevel::all);
		// Initialize Asio
		m_iServer.init_asio();
		...
		m_iServer.set_fail_handler(std::bind(&Pubisher::OnFail, this, std::placeholders::_1));
...
bool vsnc::vnet::Pubisher::Pubish(const Data& data)
{
	for (auto& hdl : m_lstHandle) {
		std::error_code ec;
		m_iServer.send(hdl,data.ptr,data.len, websocketpp::frame::opcode::BINARY,ec);
		if (ec) {
...
void vsnc::vnet::Pubisher::OnClose(websocketpp::connection_hdl hdl)
{
	...
	std::cout << "colse connect" << std::endl;
}

void vsnc::vnet::Pubisher::OnFail(websocketpp::connection_hdl hdl)
{
	std::cout << "onFial" << std::endl;
}
